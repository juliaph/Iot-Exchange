...
const db = {
    createConnection() {
        return mysql.createConnection({
            host: 'mysql5013.site4now.net',
            user: '99c406_tcc',
            password: 'tccpassword1',
            database: 'db_99c406_tcc',
...
function getProfile(model) {
    var props = ['firstName', 'lastName', 'companyName', 'companyType'];
    var profile = {};
...
function grantUser(user, password, done) {
    ...
    connection.query('INSERT INTO `accesstokens` (`token`, `user_id`) SELECT ' + t + ', `id` FROM `users` WHERE `name` = ' + u + ' AND `password` = ' + p + ';', (err, rows, fields) => {
        if (err)
            done('Unspecified error generating token.');
...
function clearToken(token, done) {
    var connection = db.createConnection();
    var t = connection.escape(token);
    connection.query('DELETE FROM `accesstokens` WHERE `token` = ' + t + ';', (err, rows, fields) => {
        if (err) 
...
function getUserByToken(token, done) {
    var connection = db.createConnection();
    var t = connection.escape(token);
    connection.query('SELECT `u`.* FROM `users` AS `u` JOIN `accesstokens` AS `t` ON `u`.`id` = `t`.`user_id` WHERE `t`.`token` = ' + t + ';', (err, rows, fields) => {
        if (err)
...
function register(model, done) {
    ...
    if (model.password !== model.confirmPassword) {
        ...
        connection.query('INSERT INTO `users` (`name`, `password`, `data`) VALUES (' + name + ', ' + password + ', ' + data + ');', (err, rows, fields) => {
            if (err) {
                console.log(err);
...
function capability(done) {
    var connection = db.createConnection();
    connection.query('SELECT c.id as c_id, c.name as c_name, s.id as s_id, s.name as s_name FROM capabilities AS c JOIN stacks AS s ON c.id = s.capability_id', (err, rows, fields) => {
        if (err)
...
function getUserStacks(id, done) {
    ...
    connection.query('SELECT `s`.`id`, `s`.`name`, `us`.`data` FROM `userstacks` AS `us` JOIN `stacks` AS `s` ON `us`.`stack_id` = `s`.`id` WHERE `us`.`user_id` = ' + i + ';', (err, rows, fields) => {
        if (err) {
...
function updateUserStacks(id, stacks, done) {
    ...
    connection.query('DELETE FROM `userstacks` WHERE `user_id` = ' + u + '; INSERT INTO `userstacks` (`user_id`, `stack_id`, `data`) SELECT ' + u + ', `id`, ' + empty + ' FROM `stacks` WHERE `id` IN (' + sarr.join(', ') + ');', (err, rows, fields) => {
        if (err) {
...
function addOffering(offering, done) {
    ...
    connection.query('INSERT INTO `userstacks` (`user_id`, `stack_id`, `data`) VALUES (' + user_id + ', ' + stack_id + ', ' + data + ');', (err, rows, fields) => {
        if (err) {
            if (err.code === 'ER_DUP_ENTRY') {
...
function removeOffering(key, done) {
    ...
    connection.query('DELETE FROM `userstacks` WHERE `user_id` = ' + user_id + ' AND `stack_id` = ' + stack_id + ';', (err, rows, fields) => {
        if (err) {
...
function search(type, stacks, done) {
    ...
    var prop = connection.escape('$.companyType');
    var sarr = [];
    if (util.isArray(stacks)) {
        for (var stack of stacks)
            sarr.push(parseInt(stack));
    ...
    connection.query('SELECT * FROM `users` AS `u`'
        + ' JOIN (SELECT DISTINCT `user_id` FROM `userstacks` WHERE `stack_id` IN (' + sarr.join(', ') + ')) AS `ids`'
        + ' ON `u`.`id` = `ids`.`user_id`'
        + ' WHERE JSON_EXTRACT(`u`.`data`, ' + prop + ') = ' + t, 
        (err, rows, fields) => {
            if (err) {
